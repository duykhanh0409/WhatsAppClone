...
struct ChatPartnerPickerScreen: View {
    ...
    var body: some View {
        NavigationStack(path: $viewModel.navStack) {
                ...
                Section {
                    ForEach(viewModel.users) { user in
                        ChatPartnerRowView(user: user)
                            .onTapGesture {
                                viewModel.createDirectChannel(user) { newChannel in
                                    onCreate(newChannel)
                                }
                            }
                    }
            ...
            .alert(isPresented: $viewModel.errorState.showError, content: {
                Alert(title: Text("Un Oh"), message: Text(viewModel.errorState.errorMessage),  dismissButton: .default(Text("Oke")))
            })
            .toolbar {
                trailingNavItem()
            }
            .onAppear() {
                viewModel.deSelectAllChatParners()
            }
        }
    }
